...
typedef struct
{
    const char *prompt;   // User prompt message (NULL = end of array)
    const char *key;      // Unique identifier key
    udc_data_type_t type; // Expected data type
    bool required;        // True if field is mandatory
} udc_data_request_t;
...
static inline size_t udc_count_requests(const udc_data_request_t *requests)
{
    size_t count = 0;
...
static inline bool udc_to_bool(const char *input, bool *result)
{
    char lower[32];
...
static inline bool udc_to_hex(const char *hex_string, uint8_t *buffer, size_t *length)
{
...
static inline bool udc_validate(const char *input, udc_data_type_t type, udc_collected_data_t *result)
{
    switch (type)
    {
...
static inline udc_result_t udc_collect_single(const udc_data_request_t *request, udc_collected_data_t *result)
{
...
static inline void udc_show_summary(const udc_data_request_t *requests, size_t count, const udc_collected_data_t *results)
{
    udc_send("\r\n=== SUMMARY ===\r\n");
...
static inline void udc_collection_task(void *pvParameters)
{
...
static inline udc_collected_data_t *udc_get_auto(const char *key, const udc_data_request_t *requests, udc_collected_data_t *results)
{
 ...
 */
static inline esp_err_t udc_collect_async(const udc_data_request_t *requests,
                                          size_t count,
                                          udc_collected_data_t *results,
                                          volatile bool *done_flag,
                                          volatile udc_result_t *result_code,
 ...
 * @return ESP_OK on success, error code on failure
 */
static inline esp_err_t udc_start_auto(const udc_data_request_t *requests,
                                       udc_collected_data_t *results,
                                       volatile bool *done_flag,
                                       volatile udc_result_t *result_code)
{
 ...
 * @return ESP_OK on success, error code on failure
 */
static inline esp_err_t udc_write_to_factory(const udc_data_request_t *requests, const udc_collected_data_t *results)
{
